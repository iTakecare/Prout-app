    ...
    <div className="gauge">
      ...
      >
        <div className="inner">
          ...
          <div className="cap">Probabilité</div>
        </div>
      </div>
      <div>
        <h3 style={{ fontSize: 16 }}>{result.verdict}</h3>
        <p className="muted" style={{ margin: "4px 0 0", maxWidth: 280 }}>
          Score de faisabilité du projet de méthanisation, pondéré sur la taille
          du gisement, la qualité des substrats et le contexte du site.
        </p>
      </div>
    </div>
  );
...
export default function ResultView({ result }: { result: CalcResult }) {
  const isCogen = result.valorization === "cogeneration";
  return (
    <div className="stack">
      <div className="card">
      ...
      <div className="grid kpi-grid">
        <div className="kpi">
          <div className="label">Gisement total</div>
        ...
        <div className="kpi">
          ...
          <div className="sub">{fmtInt(result.biogasTotal)} m³ de biogaz brut</div>
        </div>
        <div className="kpi">
          <div className="label">Énergie primaire</div>
          <div className="value">{fmtEnergy(result.primaryEnergyKwh)}</div>
          <div className="sub">PCI du biogaz produit</div>
        </div>
        {isCogen ? (
          <>
            <div className="kpi">
              <div className="label">Électricité</div>
              <div className="value">{fmtEnergy(result.electricityKwh)}</div>
              <div className="sub">
                ≈ {fmtInt(result.homesEquivalent)} foyers alimentés
              </div>
            </div>
            <div className="kpi">
              <div className="label">Chaleur valorisée</div>
              <div className="value">{fmtEnergy(result.heatValorizedKwh)}</div>
              <div className="sub">
                sur {fmtEnergy(result.heatKwh)} cogénérés
              </div>
            </div>
          </>
        ) : (
          <div className="kpi">
            <div className="label">Biométhane injecté</div>
            <div className="value">{fmtEnergy(result.biomethaneKwh)}</div>
            <div className="sub">
              ≈ {fmtInt(result.homesEquivalent)} foyers chauffés
            </div>
          </div>
        )}
        <div className="kpi">
          <div className="label">Recette énergétique</div>
          <div className="value">{fmtEur(result.revenue)}/an</div>
          <div className="sub">valorisation estimée</div>
        </div>
        <div className="kpi">
          <div className="label">CO₂ évité</div>
          <div className="value">{fmtInt(result.co2Avoided)} t/an</div>
          <div className="sub">énergie fossile substituée</div>
        </div>
      </div>
        ...
        <div className="card">
          ...
          <table>
            <thead>
              <tr>
                <th>Substrat</th>
                <th className="num">Tonnage brut</th>
